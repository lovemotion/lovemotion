...
4. Courier adapter (Spaces): ~~MessagePack serialization~~ ✓ (`src/courier.lisp`, both directions); ~~transport code~~ ✓ (`src/transport.lisp` + `src/zs3-shim.lisp`, `COURIER.md` convention; test `(asdf:test-system :lovemotion/transport)`); ~~bucket + LoveMotion key~~ ✓ (sfo3, live round-trip tested 2026-07-10; creds in gitignored `.env`, load via `set -a && source .env`); ~~COURIER.md veto pass~~ ✓ (Danny, 2026-07-10, all keep); ~~batch entrypoint~~ ✓ (`src/batch.lisp`: `(lovemotion.batch:courier-batch-run)` drains twins/v1 → runs → ships matches/v1, skips the run when the courier is quiet; cursor + idempotency tables in schema; test `(asdf:test-system :lovemotion/batch-test)` — DESTRUCTIVE, needs live DB). ~~scheduling on the droplet~~ ✓ (`deploy/lovemotion-batch.{service,timer}`, nightly 03:15 UTC, first-run verified 2026-07-10; env in droplet `/etc/lovemotion/batch.env`). ~~HeyU's Spaces plumbing~~ ✓ (~~agreement~~ ✓ ~~Elixir encoder~~ ✓ ~~HeyU's scoped key~~ ✓ minted 2026-07-11; ~~30-day lifecycle rule~~ ✓ set + verified 2026-07-11 via `deploy/set-lifecycle.lisp`; first two real twin batches landed in twins/v1 night of 2026-07-11 — 1 + 17 twins, confidences 0.15–0.60, decode verified against our codec; HeyU's reader verified both directions incl. run-id as UUID *string* — was an `<int>` doc bug in COURIER.md, fixed 2026-07-12 before it bit). Go-live still needs: **first integration batch** — timer fires 03:15 UTC 2026-07-12, HeyU reads 03:45; next session: check `journalctl -u lovemotion-batch.service` shows `drained 2` and a shipped matches/v1 key. Expect sparse, low-scored matches (all confidences ≤ 0.60 → heavy composite discount) — that's correct, not a bug. NOTE: prod DB was purged of the golden fixtures + Jul 3 smoke run on 2026-07-12 (they'd have polluted the real pool; pre-purge pg_dump in gitignored `backups/`)
